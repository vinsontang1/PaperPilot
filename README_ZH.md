...
- **方式 A**：网页首页点击「📎 添加文献」按钮上传 PDF
- **方式 B**：把 PDF 文件放到 `data/origin/` 目录，然后网页上点 Rescan

### 预处理论文（与 Agent 对话）

【【档位切换截图 prompt: Screenshot showing PaperPilot's tier navigation - three tabs labeled "摘要", "精解", "原文+翻译" with the summary tab active. Below shows a Chinese summary with metadata block, key method points, and a figure placeholder rendered as an embedded image with caption. Indigo accent, clean typography.】】
...
Agent 递归阅读该分类下所有论文摘要，生成带引用链接的综述文稿。
...
Agent：✅ 已将 ThinkRec 迁移到 recommendation/llm4rec
```
...
Agent：已读取 3 篇论文摘要，正在生成引用文稿…
       ...
       访问：http://127.0.0.1:7856/#/folder/recommendation
```
...
```
PaperPilot/
├── skill/                     # Agent Skill 文档
│   ├── SKILL.md               # 入口：触发条件 + 规则 + 方法索引
│   └── methods/               # 12 个方法文件
│       ├── preprocess.md      # 预处理流程（一键脚本 + 3 agent 并行）
│       ├── classify.md        # 智能分类
│       ├── archive.md         # 批量归档
